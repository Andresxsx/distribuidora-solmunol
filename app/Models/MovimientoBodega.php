...
class MovimientoBodega extends Model
{
    ...
    protected static function booted(): void
    {
        ...
        static::saving(function (MovimientoBodega $movimiento) {
            ...
            if (! in_array($movimiento->tipo_movimiento, ['Entrada', 'Salida', 'Ajuste'])) {
                throw ValidationException::withMessages([
                    'tipo_movimiento' => 'El tipo de movimiento no es válido.',
                ]);
            }

            if (! in_array($movimiento->origen, ['Compra', 'Venta', 'Corrección'])) {
                throw ValidationException::withMessages([
            ...
            if ((int) $movimiento->stock_anterior < 0 || (int) $movimiento->stock_nuevo < 0) {
                throw ValidationException::withMessages([
                    'stock_nuevo' => 'El stock no puede quedar en negativo.',
                ]);
            }
        });
    }
